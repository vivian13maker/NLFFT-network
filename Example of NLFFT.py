...
# Modified from RFB Net（https://arxiv.org/pdf/1711.07767.pdf）、 DETR (https://github.com/facebookresearch/detr)
...
from transformer_models import NLFFT
...
class RFBNet(nn.Module):
    ...
    def __init__(self, phase, size, base, extras,ft,
                 head, adj_module, num_classes,  NLFFT, po_e):
        ...
        self.NLFFT = NLFFT
        self.po_e = po_e
        self.ft =nn.ModuleList(ft)
        self.adj_module = nn.ModuleList(adj_module)
        if size == 300:
            self.indicator = 3
        elif size == 512:
            self.indicator = 5
    ...
    def forward(self, x):
        ...
        for k in range(23):
            x = self.base[k](x)
        c43_feature = self.adj_module[0](x)
        s = self.Norm(x)
        sources.append(s)
        ...
        for k in range(23, len(self.base)):
            x = self.base[k](x)
        fc7_feature = self.adj_module[1](x)
        fc7_feature = F.interpolate(fc7_feature, 38, mode='bilinear')

        # transformer_process
        bs, c, h, w = fc7_feature.shape

        mask_device = x.device
        # c43 + fc7
        mask = torch.ones((x.shape[0], c43_feature.shape[2], c43_feature.shape[3]), dtype=torch.bool,
                          device=mask_device)
        ...
        ipo43 = misc.NestedTensor(fc7_feature, mask)
        po43 = self.po_e(ipo43)

        hs = self.NLFFT(c43_feature, mask, fc7_feature, po43)[0]
        hs0_feature = hs[-1].permute(0, 2, 1).view(bs, c, h, w)
        hs0_feature = F.relu(hs0_feature)
        ...
        for k, v in enumerate(self.extras):
            ...
            if k < self.indicator or k % 2 == 0:
                sources.append(x)
        for i, v in enumerate(self.ft):
            new_fea = v(hs0_feature)
            sources.append(new_fea)
        # if k == 7
        # layers = [BasicConv(256, 512, kernel_size=3, stride=1, padding=1)]
        # for i, v in enumerate(self.pyramid_ext):
        #     new_fea = v(hs0_feature)
        # sources.append(new_fea)
...
def feature_transform_module(vgg, size):
    if size == 300:
        up_size = 38
    elif size == 512:
        up_size = 64
    ...
    # fc_7
    layers += [BasicConv(vgg[-2].out_channels, 256, kernel_size=1, padding=0)]
    return layers
...
def ft_extras(size,batch_norm = False ):
    layers = []
    if size == 300:
        layers += [BasicConv(256, 512, kernel_size=3, stride=1, padding=1)]
    elif size == 512:
...
def multibox(size, vgg, extra_layers, cfg, num_classes, ft_layers):
    loc_layers = []
    conf_layers = []
    vgg_source = [-2]
    # 对VGG网络结构中conv2d-7-1通过卷积提取特征
    ...
    for k, v in enumerate(extra_layers):
        if k < indicator or k % 2 == 0:
            ...
            i += 1
    for k, v in enumerate(ft_layers):
        loc_layers += [nn.Conv2d(v.out_channels, cfg[i] * 4, kernel_size=3, padding=1)]
        conf_layers += [nn.Conv2d(v.out_channels, cfg[i]
                                  * num_classes, kernel_size=3, padding=1)]
        i += 1
    return vgg, extra_layers, ft_layers, (loc_layers, conf_layers)
...
mbox = {
    '300': [6, 6, 6, 6, 4, 4, 6],  # number of boxes per feature map location
    '512': [6, 6, 6, 6, 6, 4, 4],
}
...
def build_net(args, phase, size=300, num_classes=21):
    ...
    return RFBNet(phase, size, *multibox(size, vgg(base[str(size)], 3),
                                         ...
                                         mbox[str(size)], num_classes, ft_extras(size)),
                  adj_module=feature_transform_module(vgg(base[str(size)], 3), 300), num_classes=num_classes,
                  transformer_a=transformer.build_transformer(args), po_e=position_embedding)
